// ============================================================
// POS module — embedded into kasir.html as the "POS" + "Riwayat" tabs.
// Wrapped in an IIFE so its globals (currentDate, esc, checkAuth,
// renderSummary, ...) do NOT collide with kasir.js. Only initPOS(),
// loadRiwayat() and openRefund() are exposed on window.
// ============================================================
(function () {
  let cart = [];
  let payMethod = 'cash';
  ...
  function pickBarber(el) {
    ...
    const tag = document.getElementById('cart-barber-tag');
    tag.textContent = selectedBarber.name;
    tag.style.display = '';
  }
  ...
  function updateCartUI() {
    ...
    if (bar) {
      ...
      const cnt = document.getElementById('mobile-cart-count');
      const itm = document.getElementById('mobile-cart-items');
      const tot = document.getElementById('mobile-cart-total');
      if (cnt) cnt.textContent = qty;
      if (itm) itm.textContent = qty + ' item' + (selectedBarber ? ' • ' + selectedBarber.name : '');
  ...
  async function loadRiwayat() {
    const dateEl = document.getElementById('riwayat-date');
    const date = (dateEl && dateEl.value) || posDate;
    try {
      ...
      if (sumData.success) renderOmset(sumData.data);
      renderRiwayat();
    } catch {}
  }

  function renderOmset(s) {
  ...
  function renderRiwayat() {
    ...
    list.innerHTML = filtered.map(t => {
      ...
      return '<div class="trx-card' + (isRefunded ? ' refunded' : '') + '">' +
        ...
        (!isRefunded ? '<button class="trx-btn refund-btn" data-refund-id="' + esc(t.id) + '"><i class="fa-solid fa-rotate-left"></i> Refund</button>' : '') +
        '</div></div></div>';
    }).join('');

    list.querySelectorAll('[data-refund-id]').forEach(btn => {
      btn.addEventListener('click', () => openRefund(btn.dataset.refundId));
    });
  }
  ...
  // ============================================
  // INIT — called once when the POS tab is first opened.
  // ============================================
  async function initPOS() {
    ...
    const now = new Date();
    posDate = now.getFullYear() + '-' + String(now.getMonth() + 1).padStart(2, '0') + '-' + String(now.getDate()).padStart(2, '0');
    const rdate = document.getElementById('riwayat-date');
    if (rdate && !rdate.value) rdate.value = posDate;
  ...
  // Expose the pieces kasir.js / the riwayat list need.
